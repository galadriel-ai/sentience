...
class AttestationProof:
    def __init__(self, hashed_data: bytes, signature: Signature, public_key: Pubkey, attestation: bytes):
        self.schema = CStruct("hashed_data" / U8[32], "signature" / U8[64], "public_key" / U8[32], "attestation" / U8[32])
    ...
    def serialize(self):
        return self.schema.build({"hashed_data": self.hashed_data, "signature": bytes(self.signature), "public_key": bytes(self.public_key), "attestation": self.attestation})
...
class ContractClient:
    ...
    async def call_instruction(self, signer: list[Keypair], data: bytes, accounts: list[AccountMeta]):
        ...
        return await self.client.send_transaction(transaction, opts=TxOpts(skip_confirmation=False, preflight_commitment=Confirmed))
    ...
    async def initialize_program(self):
        ...
        return await self.call_instruction([self.keypair], bytes(settings.INSTRUCTION_DISCRIMINATORS["initialize"]), accounts)
    ...
    async def add_authority(self, authority: Pubkey):
        ...
        return await self.call_instruction([self.keypair], bytes(settings.INSTRUCTION_DISCRIMINATORS["add_authority"]), accounts)
    ...
    async def remove_authority(self, authority: Pubkey):
        ...
        return await self.call_instruction([self.keypair], bytes(settings.INSTRUCTION_DISCRIMINATORS["remove_authority"]), accounts)
    ...
    async def add_proof(self, proof: AttestationProof):
        ...
        proof_record_pda = Pubkey.find_program_address([b"attestation", proof.hashed_data], PROGRAM_ID)[0]
        accounts = [
        ...
        data = bytes(settings.INSTRUCTION_DISCRIMINATORS["add_proof"]) + proof.serialize()
...
async def main():
    ...
    response = await client.add_proof(AttestationProof(array_of_32_bytes, array_of_64_bytes, array_of_32_bytes, array_of_32_bytes)) 
    print(response)
